...
class Button extends StatelessWidget {
  ...
  const Button({
    ...
    this.fontSize = 24,      // Kích thước chữ mặc định là 18.0
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor: color,  // Tùy chỉnh màu sắc
        minimumSize: const Size(double.infinity, 45), // Chiều cao nút là 60 và chiều rộng tự động
        padding: const EdgeInsets.symmetric(vertical: 16.0), // Tùy chỉnh padding (có thể thay đổi theo nhu cầu)
      ),
      child: Text(
        text,
        style: TextStyle(fontSize: fontSize),  // Điều chỉnh kích thước chữ
      ),
    );
  }
}
